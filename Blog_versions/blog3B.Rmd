...
relate heat accumulation after winter to pest emergence; while sampling relies on on-site sampling which reflect
pest population densities to inform decisions about controlling pest populations. A drawback of existing methods is that phenology and
sampling remain unlinked in the models; growers must rely on their intuition and experiences to
connect the dots. To solve this, my collegues Diego Rincon, David Crowder, and I developed a methodology for
creating site-specific predictions that link phenology models to sampling data by adjusting their shape. However, we still needed a
statistical procedure to use these "profiles" to allow growers to see the probability of exceeding their maximum tolerable
pest density given their early-season sampling data. This started an offshoot of our work, which culminated in
publishing "Sequential testing of complementary hypotheses about population density" in MEE about our new statistical method,
...
Data for pest sampling comes in week-to-week in batches, so rather than traditional hypothesis testing tools which consider
...
explicit threshold-style hypotheses, and even dynamic trajectories of hypotheses.
...
For example, by continuously comparing the expected patient mortality
rates for certain surgical procedures to that of individual
surgeons, we found that [Harold Shipman's serial killings](https:// en.wikipedia.org/wiki/Harold_Shipman) could have been detected
